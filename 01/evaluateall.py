...
Python's in built sorting function is the fastest, being at
least two orders of magnitude faster than any other function

Mergesort is an order of magnitude faster than either
heapsort or quicksort, with quicksort being regularily
    ...
    for test_list in list_dict:
        for test_sort in sort_list:
            if n > 10000 and test_sort.__name__ == 'bubblesort':
                print('Skipping bubblesort for this value')
                continue
            result = test(test_sort, list_dict[test_list])
            if type(result) == str:
                print (result)
            else:
                print('%-2.9f %-8s %-10s %5d %3d %3d' % (
